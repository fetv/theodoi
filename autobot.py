        ...
        ydl_opts = {
            ...
            "extractor_args": {
                "youtube": {
                    ...
                    "player_client": ["all", "-android", "-ios", "-tv", "-html5", "-web", "-android_sdkless", "-android_vr", "-tv_simply", "-tv_embedded"],
    ...
    try:
        ...
        if channel_events[channel]:
            ...
            if wait_for_render is False:
                return False
        real_video_path = video_path if is_rendered[channel] and is_rendered[channel] == video_id else download_path
    ...
    with sync_playwright() as p:
        try:
            ...
            absolute_file_path = os.path.join(
                os.getcwd(),
                video_path if is_rendered[channel] and is_rendered[channel] == video_id else download_path,
            )
...
def process_video_pipeline(channel, video):
    # global is_rendered
    channel_configs = ALL_CONFIGS[channel]["config"]
    ...
    VIDEO_OUT = Path(f"processed/{file_name}.mp4")
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        try:
            ...
            download_future = executor.submit(
                download_video, channel, video_url, download_path
            )
            # Wait for download session
            try:
                download_result = download_future.result(timeout=100)
            ...
            download_time = download_finish_time - start_time
            print(f"Thời gian download: {download_time:.2f} giây")

            # Start render video setup
            render_video_method = channel_configs.get("render_video_method", "repeat")
            render_future = executor.submit(
                render_video_ffmpeg, download_path, VIDEO_OUT, render_video_method
            )
            # Wait for render video session
            try:
                rendered = render_future.result(timeout=20)
                if rendered is False:
                    is_rendered[channel] = None
                    raise Exception(ERROR_MSG)
                elif rendered is True:
                    is_rendered[channel] = video.id
                else:
                    is_rendered[channel] = None
                
            except Exception as e:
                err_msg = (
                    f"✅ Lỗi trong khi render video\n{video_title}\n{video_url}: {e}"
                )
                print(err_msg)
                send_telegram_message(channel, err_msg)
                raise
            render_finish_time = time.perf_counter()
            render_time = render_finish_time - download_finish_time
            print(f"Thời gian render video: {render_time:.2f} giây")
            time.sleep(0.01)
            with event_lock:
                channel_events[channel].set()
            # Wait for upload tiktok
            try:
                post_result = upload_future.result(timeout=60)
                if not post_result:
                    raise Exception(ERROR_MSG)
            except Exception as e:
                ...
                send_telegram_message(channel, err_msg)
                raise
            post_finish_time = time.perf_counter()
            post_tiktok_duration = post_finish_time - render_finish_time
            print(f"Thời gian đăng tiktok: {post_tiktok_duration:.2f} giây")
            process_duration = post_finish_time - start_time
            current_time_tiktok = datetime.now().strftime("%H:%M:%S")
            current_time = datetime.now().strftime("%H:%M:%S")
            process_time_str = f"Tổng download+render+upload: {process_duration:.2f}s\nDownload:{download_time:.2f}s\nRender:{render_time:.2f}s\nUpload:{post_tiktok_duration:.2f}s"
            print(f"✅{current_time}:Đăng video thành công\n{process_time_str}\n")
            username = channel_configs.get("username", "default_username")
            upload_method = channel_configs.get("upload_method", "browser")
            tele_msg = f"✅ Đã đăng TikTok thành công ({upload_method}):\n{video_title}\n{video_url}\n{process_time_str}\nTài khoản TikTok: {username}\nĐăng lúc: {current_time_tiktok}\n"
            send_telegram_message(channel, tele_msg)
            return True
        except Exception:
            # Cancel all futures if any step fails
            for future in [download_future, render_future, upload_future]:
                if future and future.running():
                    future.cancel()
            raise  # Re-raise to ensure cleanup in outer block

        finally:
            # Clean up temporary files
            try:
                download_path.unlink(missing_ok=True)
                if is_rendered[channel] is True:
...
async def main():
    ...
    for channel in (
        ...
        print(f"Quét kênh {channel} mỗi {interval} giây")

    print(f"🟢 Khởi chạy ứng dụng lúc {datetime.now().strftime('%H:%M:%S %d/%m/%Y')}")
